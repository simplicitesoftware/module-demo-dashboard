...
/**
 * Dashboard external object
 * @class
 */
Simplicite.UI.ExternalObjects.DemoDashboard = class extends Simplicite.UI.ExternalObject {
	/** @override */
	async render(_params, data) {
		// These two charts are using a "select" object
		const charts1and2 = async (_d1, _d2) => {
			$ui.getUIObject('DemoStats2', 'dashboard_DemoStats2', async sts => {
		...
		const chart3 = async _d => {
			$ui.getUIObject('DemoOrder', 'dashboard_DemoOrder', async ord => {
				...
				const cubes = await ord.getPivotTableData(tab.name);
		...
		const chart4 = async _d => {
			$ui.getUIObject('DemoStats1', 'dashboard_DemoStats1', async sts => {
